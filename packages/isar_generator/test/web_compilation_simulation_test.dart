...
''';
      ...
      expect(generatedCode, contains(
          'idGenerator: () => _webSafeHash(r\'ScheduledCommandEntity\')'));
      expect(generatedCode,
          contains('idGenerator: () => _webSafeHash(r\'commandType\')'));
      expect(generatedCode,
          contains('idGenerator: () => _webSafeHash(r\'createdAt\')'));
    });
...
''';
      ...
      expect(generatedCode,
          contains('& 0x7FFFFFFF')); // JavaScript safe integer mask
...
''';
      ...
      for (final match in numericLiterals) {
        ...
        if (value != null && value > 0x1FFFFFFFFFFFFF) { // 2^53 - 1
          fail(
              'Found numeric literal $value that exceeds JavaScript safe integer range');
        }
      }

      // Verify all schemas use function-based ID generation
      expect(generatedCode,
          contains('idGenerator: () => _webSafeHash(r\'Person\')'));
      expect(generatedCode,
      ...
      expect(generatedCode,
          contains('idGenerator: () => _webSafeHash(r\'friends\''));
    });
...
''';
    ...
    test(
        'stress test with many collections and complex relationships', () async {
      const source = '''
import 'package:isar/isar.dart';
